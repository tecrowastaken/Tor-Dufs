...
use tor_utils::setup_tor_daemon;
...
async fn main() -> Result<()> {
    ...
    println!("CHECKING IF TOR DAEMON SHOULD BE USED : {}", args.enable_hs);
    let maybe_tor_daemon = match args.enable_hs {
        true => {Some(setup_tor_daemon(&args)?)},
        false => {None}
    };
    println!("DONE CHECKING FOR DAEMON");
