...
pub fn build_cli() -> Command {
    let app = Command::new(env!("CARGO_CRATE_NAME"))
        ...
        // Tor-support this part is our code...
        .arg(
            Arg::new("enable-hs")
                .env("DUFS_ENABLE_TOR_HIDDEN_SERVICE")
                .hide_env(true)
                .long("enable-hs")
                .short('T')
                .action(ArgAction::SetTrue)
                .help("Enables working hosting as a Tor Hidden Service [default: false]")
        )
        .arg(
            Arg::new(
                "hs-path"
            ).env("DUFS_TOR_HIDDEN_SERVICE_DIRECTORY")
            .hide_env(true)
            .long("hs-path")
            .value_parser(value_parser!(PathBuf))
            .help("Establishes Where the path to your hidden service should go, WARNING! IT DEFAULTS TO THE SAME DIRECTORY AS THE FILEPATH CHOSEN BE CAREFUL WHEN SETTING THIS UP!!!")
        )
        .arg(
            Arg::new(
                "tor-client-port"
            ).env(
                "DUFS_TOR_CLIENT_PORT"
            )
            .hide_env(true)
            .value_parser(value_parser!(u16))
            .long("tor-client-port")
            .help("Port to use when setting up your tor config port [default: 9050]")
        )
        .arg(
            Arg::new(
                "tor-socket-port"
            )
            .env("DUFS_TOR_SOCKET_PORT")
            .hide_env(true)
            .value_parser(value_parser!(u16))
            .long("tor-socket-port")
            .help("The Socket Port to use when working with the dufs server if you have tor enabled useful if you need to interact using a program of some kind [default: 9051]")
        )
        .arg(
            Arg::new(
                "tor-password"
            )
            .env("DUFS_TOR_PASSWORD")
            .hide_env(true)
            .long("tor-password")
            .help("If used will pass a password to use for your tor configuration. Know that not having one is discouraged")
        );
...
pub struct Args {
    ...
    // Our added features are here
    pub enable_hs:bool,
    pub hs_path:Option<PathBuf>,
    #[default(9050)]
    pub tor_client_port:u16,
    #[default(9051)]
    pub tor_socket_port:u16,
...
impl Args {
    ...
    pub fn parse(matches: ArgMatches) -> Result<Args> {
        ...
        if !args.enable_hs {
            args.enable_hs = matches.get_flag("enable-hs");
        }

        if let Some(hs_path) = matches.get_one::<PathBuf>("hs-path"){
            args.hs_path = Some(hs_path.to_path_buf());
        }

        if let Some(tor_client_port) = matches.get_one::<u16>("tor-client-port") {
            args.tor_client_port = *tor_client_port
        }

        if let Some(tor_socket_port) = matches.get_one::<u16>("tor-socket-port") {
            args.tor_socket_port = *tor_socket_port;
        }

        if let Some(tor_password) = matches.get_one::<String>("tor-password") {
            args.tor_password = Some(tor_password.clone());
        }
